	...
	methods: {
		...
		initSchoolData() {
			...
			try {
				...
				if (userInfo.mathScore >= 0) {
					this.selectNumber = Number(userInfo.mathScore);
					this.selectNumberText = numberList.find((el) => {
						return el.value == userInfo.mathScore;
					}).name;
				}

				if (userInfo.textBookName > 0 && userInfo.textBookName <= 5) {
					this.selectBook = Number(userInfo.textBookName);
					this.selectBookText = bookList.find(
						(el) => el.value == userInfo.textBookName
					).name;
					this.selectBookDisabled = true;
					console.log('this.selectBookText', this.selectBookText);
		...
		onSaveSchoolInfo() {
			...
			let schoolGradeId = Number(this.selectGrade);
			this.$api
				.updateSchool({
					schoolId,
					textBook,
